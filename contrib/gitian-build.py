...
def setup():
    ...
    if not os.path.isdir('gitian.sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/Vegas-Guy/gitian.sigs.git'])
    if not os.path.isdir('PIVX-detached-sigs'):
        subprocess.check_call(['git', 'clone', 'https://github.com/Vegas-Guy/LasVegasCoin2-detached-sigs.git'])
    if not os.path.isdir('gitian-builder'):
        subprocess.check_call(['git', 'clone', 'https://github.com/devrandom/gitian-builder.git'])
    if not os.path.isdir('PIVX'):
        subprocess.check_call(['git', 'clone', 'https://github.com/Vegas-Guy/LasVegasCoin2.git'])
...
def build():
    ...
    subprocess.check_call(['make', '-C', '../LasVegasCoin2/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
    ...
    if args.linux:
        ...
        subprocess.check_call('mv build/out/lasvegascoin2-*.tar.gz build/out/src/lasvegascoin2-*.tar.gz ../PIVX-binaries/'+args.version, shell=True)
    ...
    if args.windows:
        ...
        subprocess.check_call('mv build/out/lasvegascoin2-*-win-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/lasvegascoin2-*.zip build/out/lasvegascoin2-*.exe ../lasvegascoin2-binaries/'+args.version, shell=True)
    ...
    if args.macos:
        ...
        subprocess.check_call('mv build/out/lasvegascoin2-*-osx-unsigned.tar.gz inputs/', shell=True)
        subprocess.check_call('mv build/out/lasvegascoin2-*.tar.gz build/out/lasvegascoin2-*.dmg ../lasvegascoin2-binaries/'+args.version, shell=True)
...
def sign():
    ...
    if args.windows:
        ...
        subprocess.check_call('mv build/out/pivx-*win64-setup.exe ../lasvegascoin2-binaries/'+args.version, shell=True)
        subprocess.check_call('mv build/out/pivx-*win32-setup.exe ../lasvegascoin2-binaries/'+args.version, shell=True)
    ...
    if args.macos:
        ...
        subprocess.check_call('mv build/out/lasvegascoin2-osx-signed.dmg ../lasvegascoin2-binaries/'+args.version+'/PIVX-'+args.version+'-osx.dmg', shell=True)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/Vegas-Guy/LasVegasCoin2', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.chdir('LasVegasCoin2')
    if args.pull:
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
        os.chdir('../gitian-builder/inputs/lasvegascoin2')
        subprocess.check_call(['git', 'fetch', args.url, 'refs/pull/'+args.version+'/merge'])
